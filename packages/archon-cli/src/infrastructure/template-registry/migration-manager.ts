...
import { globalCache, templateResolver } from '../cache/index.js';
...
export class MigrationManager {
  ...
  private loadChangelog(from: string, to: string): string | undefined {
    ...
    try {
      ...
      let capturing = false;
      const fromPattern = new RegExp('^##?\\s*\\[?v?"?' + from.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + ']?', 'i');
      const toPattern = new RegExp('^##?\\s*\\[?v?"?' + to.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + ']?', 'i');

      for (const line of lines) {
        if (fromPattern.test(line) || toPattern.test(line)) {
          capturing = !capturing;
          if (!capturing) break;
          continue;
        }
  ...
  updateTemplateLock(newVersion: string): void {
    ...
    const templateId = lock.template.id;

    if (!globalCache.isInstalled(templateId, newVersion)) {
      throw new Error(
        'Template ' + templateId + '@' + newVersion + ' is not in cache.\n' +
        'Run `archon templates pull ' + templateId + '@' + newVersion + '` first.'
      );
    }

    const registry = globalCache.getRegistry();
    const versionInfo = registry[templateId]?.versions[newVersion];
    const cachePath = globalCache.getTemplatePath(templateId, newVersion);
    ...
    const updatedLock: TemplateLock = {
      ...
      template: {
        ...
        version: newVersion,
        ref: 'v' + newVersion,
        source: versionInfo?.source ?? lock.template.source,
