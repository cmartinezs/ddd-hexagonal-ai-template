...
import type { AgentSupportTier } from '../../domain/project-state/project-state.types.js';

export const AGENT_TIERS: Record<string, AgentSupportTier> = {
  opencode: 'supported',
  claude: 'supported',
  manual: 'prompt-only',
  cursor: 'planned',
...
export class AgentAdapterFactory {
  ...
  static getTier(agentId: string): AgentSupportTier {
    return AGENT_TIERS[agentId] ?? 'planned';
  }
